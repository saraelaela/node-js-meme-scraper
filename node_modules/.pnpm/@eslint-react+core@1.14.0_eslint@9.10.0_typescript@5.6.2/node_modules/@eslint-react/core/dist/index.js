...
function isInitializedFromReact(name, initialScope, settings) {
  ...
  const maybeLatestDef = tools.O.flatMapNullable(VAR__namespace.findVariable(name, initialScope), (v) => v.defs.at(-1));
  ...
  if (node.type === types.AST_NODE_TYPES.VariableDeclarator && node.init) {
    ...
    const maybeRequireExpression = tsPattern.match(init).with({
      type: types.AST_NODE_TYPES.CallExpression,
      callee: { type: types.AST_NODE_TYPES.Identifier, name: "require" }
    ...
    if (firstArg?.type !== types.AST_NODE_TYPES.Literal || !tools.isString(firstArg.value)) return false;
    return firstArg.value === importSource || firstArg.value.startsWith(`${importSource}/`);
  }
  return tsPattern.isMatching({ type: "ImportDeclaration", source: { value: importSource } }, parent);
}

// src/internal/is-from-react.ts
...
function isInsideCreateElementProps(node, context) {
  return tools.F.pipe(
    ...
    tools.O.filter(AST9__namespace.is(types.AST_NODE_TYPES.CallExpression)),
    tools.O.flatMapNullable((c) => c.arguments.at(1)),
    tools.O.filter(AST9__namespace.is(types.AST_NODE_TYPES.ObjectExpression)),
    tools.O.zipWith(AST9__namespace.traverseUp(node, AST9__namespace.is(types.AST_NODE_TYPES.ObjectExpression)), (a, b) => a === b),
    tools.O.getOrElse(tools.F.constFalse)
  );
}
function isChildrenOfCreateElement(node, context) {
  return tools.F.pipe(
    tools.O.fromNullable(node.parent),
    tools.O.filter(AST9__namespace.is(types.AST_NODE_TYPES.CallExpression)),
    tools.O.filter((n) => isCreateElementCall(n, context)),
    tools.O.exists(
...
function isReactHook(node) {
  return tools.F.pipe(
    ...
    tools.O.flatMapNullable((id) => id.name),
    tools.O.exists(isReactHookName)
  );
}
...
var isUseCallbackCall = tools.F.flip(isReactHookCallWithName)("useCallback");
var isUseContextCall = tools.F.flip(isReactHookCallWithName)("useContext");
var isUseDebugValueCall = tools.F.flip(isReactHookCallWithName)("useDebugValue");
var isUseDeferredValueCall = tools.F.flip(isReactHookCallWithName)("useDeferredValue");
var isUseEffectCall = tools.F.flip(isReactHookCallWithName)("useEffect");
var isUseIdCall = tools.F.flip(isReactHookCallWithName)("useId");
var isUseImperativeHandleCall = tools.F.flip(isReactHookCallWithName)("useImperativeHandle");
var isUseInsertionEffectCall = tools.F.flip(isReactHookCallWithName)("useInsertionEffect");
var isUseLayoutEffectCall = tools.F.flip(isReactHookCallWithName)("useLayoutEffect");
var isUseMemoCall = tools.F.flip(isReactHookCallWithName)("useMemo");
var isUseReducerCall = tools.F.flip(isReactHookCallWithName)("useReducer");
var isUseRefCall = tools.F.flip(isReactHookCallWithName)("useRef");
var isUseStateCall = tools.F.flip(isReactHookCallWithName)("useState");
var isUseSyncExternalStoreCall = tools.F.flip(isReactHookCallWithName)("useSyncExternalStore");
var isUseTransitionCall = tools.F.flip(isReactHookCallWithName)("useTransition");

// src/hook/hierarchy.ts
function isInsideReactHook(node) {
  return tools.O.exists(AST9__namespace.traverseUpGuard(node, AST9__namespace.isFunction), isReactHook);
}
function isInsideReactHookCall(node) {
  return tools.O.isSome(AST9__namespace.traverseUp(node, (n) => AST9__namespace.is(types.AST_NODE_TYPES.CallExpression)(n) && isReactHookCall(n)));
}
...
function useHookCollector() {
  ...
  const onFunctionEnter = (node) => {
    ...
    const name = tools.O.flatMapNullable(id, (id2) => id2.name);
    const isHook = tools.O.isSome(id) && tools.O.isSome(name) && isReactHookName(name.value);
    if (!isHook) {
      fStack.push([node, tools.O.none()]);
      return;
    }
    const key = uid.rnd();
    fStack.push([node, tools.O.some(key)]);
    hooks.set(key, {
      _: key,
      id,
      kind: "function",
  ...
  const listeners = {
    ...
    "CallExpression[type]"(node) {
      ...
      tools.F.pipe(
        tools.O.Do,
        tools.O.bind("id", () => hookId),
        tools.O.bind("hook", ({ id }) => tools.O.fromNullable(hooks.get(id))),
        tools.O.map(({ id, hook }) => {
          hooks.set(id, {
            ...hook,
            hookCalls: [
...
function getFunctionComponentIdentifier(node, context) {
  ...
  if (tools.O.isSome(functionId)) return functionId;
  const { parent } = node;
  if (parent.type === types.AST_NODE_TYPES.CallExpression && isComponentWrapperCall(parent, context) && parent.parent.type === types.AST_NODE_TYPES.VariableDeclarator && parent.parent.id.type === types.AST_NODE_TYPES.Identifier) {
    return tools.O.some(parent.parent.id);
  }
  if (parent.type === types.AST_NODE_TYPES.CallExpression && isComponentWrapperCall(parent, context) && parent.parent.type === types.AST_NODE_TYPES.CallExpression && isComponentWrapperCall(parent.parent, context) && parent.parent.parent.type === types.AST_NODE_TYPES.VariableDeclarator && parent.parent.parent.id.type === types.AST_NODE_TYPES.Identifier) {
    return tools.O.some(parent.parent.parent.id);
  }
  return tools.O.none();
}
  ...
  const collect = (node) => {
    ...
    components.set(
      key,
      {
        ...
        id,
        kind: "class",
        name: tools.O.flatMapNullable(id, (n) => n.name),
        node,
        // TODO: Get displayName of class component
        displayName: tools.O.none(),
        flag,
        hint: 0n,
...
function isInsideRenderMethod(node) {
  ...
  return tools.O.isSome(AST9__namespace.traverseUp(node, predicate));
}
function hasNoneOrValidComponentName(node, context) {
  return tools.O.match(
    getFunctionComponentIdentifier(node, context),
    {
      onNone: tools.F.constTrue,
      onSome: (id) => {
...
function useComponentCollector(context, hint = DEFAULT_COMPONENT_HINT) {
  ...
  const components = /* @__PURE__ */ new Map();
  const functionStack = [];
  const getCurrentFunction = () => tools.O.fromNullable(functionStack.at(-1));
  ...
  const onFunctionExit = () => {
    ...
    const shouldDrop = AST9__namespace.getNestedReturnStatements(fn.body).slice().reverse().some((r) => {
      return context.sourceCode.getScope(r).block === fn && r.argument !== null && !JSX3__namespace.isJSXValue(r.argument, jsxCtx, hint);
    });
  ...
  const listeners = {
    ...
    "ArrowFunctionExpression[type][body.type!='BlockStatement']"() {
      ...
      if (tools.O.isNone(maybeCurrentFn)) return;
      const [_key, currentFn, _isComponent, hookCalls] = maybeCurrentFn.value;
      const { body } = currentFn;
      const isComponent = hasNoneOrValidComponentName(currentFn, context) && JSX3__namespace.isJSXValue(body, jsxCtx, hint) && hasValidHierarchy(currentFn, context, hint);
      ...
      const name = tools.O.flatMapNullable(id, getComponentNameFromIdentifier);
      const key = uid3.rnd();
      components.set(key, {
        _: key,
        id,
    ...
    "AssignmentExpression[type][operator='='][left.type='MemberExpression'][left.property.name='displayName']"(node) {
      ...
      const maybeComponentName = tsPattern.match(left.object).with({ type: types.AST_NODE_TYPES.Identifier }, (n) => tools.O.some(n.name)).otherwise(tools.O.none);
      if (tools.O.isNone(maybeComponentName)) return;
      const component = Array.from(components.values()).findLast(({ name }) => tools.O.exists(name, (n) => n === maybeComponentName.value));
      if (!component) return;
      components.set(component._, {
        ...component,
        displayName: tools.O.some(right)
      });
    },
    ...
    "ReturnStatement[type]"(node) {
      ...
      if (isKnown) return;
      const isComponent = hasNoneOrValidComponentName(currentFn, context) && JSX3__namespace.isJSXValue(node.argument, jsxCtx, hint) && hasValidHierarchy(currentFn, context, hint);
      if (!isComponent) return;
      functionStack.pop();
...
function isCleanupFunction(node) {
  const nearestRet = tools.O.getOrNull(AST9__namespace.traverseUpGuard(node, AST9__namespace.is(types.AST_NODE_TYPES.ReturnStatement)));
  if (!nearestRet) return false;
  const nearestFunction = tools.O.getOrNull(AST9__namespace.traverseUpGuard(node, AST9__namespace.isFunction));
  const nearestFunctionOfRet = tools.O.getOrNull(AST9__namespace.traverseUpGuard(nearestRet, AST9__namespace.isFunction));
...
var PHASE_RELEVANCE = birecord__default.default({
  mount: "unmount",
  setup: "cleanup"
});
var isInversePhase = tools.F.dual(2, (a, b) => PHASE_RELEVANCE.get(a) === b);
function getPhaseKindOfFunction(node) {
  return tsPattern.match(node).when(isSetupFunction, () => tools.O.some("setup")).when(isCleanupFunction, () => tools.O.some("cleanup")).when(isComponentDidMountFunction, () => tools.O.some("mount")).when(isComponentWillUnmountFunction, () => tools.O.some("unmount")).otherwise(tools.O.none);
}
...
function isDeclaredInRenderPropLoose(node) {
  ...
  return tools.F.pipe(
    ...
    tools.O.flatMapNullable((c) => c.parent),
    tools.O.filter(AST9__namespace.is(types.AST_NODE_TYPES.JSXAttribute)),
    tools.O.flatMapNullable((a) => a.name),
    tools.O.exists(tsPattern.isMatching({
...
function isRenderFunctionLoose(node, context) {
  ...
  if (tools.O.isSome(maybeId) && !maybeId.value.name.startsWith("render")) {
  ...
  return JSX3__namespace.isJSXValue(
    body,
    jsxCtx,
    JSX3__namespace.JSXValueHint.SkipNullLiteral | JSX3__namespace.JSXValueHint.SkipUndefinedLiteral | JSX3__namespace.JSXValueHint.StrictLogical | JSX3__namespace.JSXValueHint.StrictConditional
  );
}
...
exports.PHASE_RELEVANCE = PHASE_RELEVANCE;
...
exports.getPhaseKindOfFunction = getPhaseKindOfFunction;
