...
import * as JSX3 from '@eslint-react/jsx';
import { F, O, isString } from '@eslint-react/tools';
import { AST_NODE_TYPES } from '@typescript-eslint/types';
import ShortUniqueId from 'short-unique-id';
import { isMatching, P, match } from 'ts-pattern';
import { unsafeReadSettings, decodeSettings } from '@eslint-react/shared';
...
function isInitializedFromReact(name, initialScope, settings) {
  ...
  if (node.type === AST_NODE_TYPES.VariableDeclarator && node.init) {
    ...
    const maybeRequireExpression = match(init).with({
...
function useComponentCollector(context, hint = DEFAULT_COMPONENT_HINT) {
  ...
  const onFunctionExit = () => {
    ...
    const shouldDrop = AST9.getNestedReturnStatements(fn.body).slice().reverse().some((r) => {
      return context.sourceCode.getScope(r).block === fn && r.argument !== null && !JSX3.isJSXValue(r.argument, jsxCtx, hint);
    });
  ...
  const listeners = {
    ...
    "ArrowFunctionExpression[type][body.type!='BlockStatement']"() {
      ...
      const isComponent = hasNoneOrValidComponentName(currentFn, context) && JSX3.isJSXValue(body, jsxCtx, hint) && hasValidHierarchy(currentFn, context, hint);
    ...
    "ReturnStatement[type]"(node) {
      ...
      if (isKnown) return;
      const isComponent = hasNoneOrValidComponentName(currentFn, context) && JSX3.isJSXValue(node.argument, jsxCtx, hint) && hasValidHierarchy(currentFn, context, hint);
      if (!isComponent) return;
      functionStack.pop();
...
var PHASE_RELEVANCE = birecord({
  mount: "unmount",
  setup: "cleanup"
});
var isInversePhase = F.dual(2, (a, b) => PHASE_RELEVANCE.get(a) === b);
...
function isRenderFunctionLoose(node, context) {
  ...
  return JSX3.isJSXValue(
    body,
    jsxCtx,
    JSX3.JSXValueHint.SkipNullLiteral | JSX3.JSXValueHint.SkipUndefinedLiteral | JSX3.JSXValueHint.StrictLogical | JSX3.JSXValueHint.StrictConditional
  );
}
