import { F, O, zip, isString } from '@eslint-react/tools';
...
function getFunctionIdentifier(node) {
  ...
  if (node.parent.type === AST_NODE_TYPES.VariableDeclarator && node.parent.init === node && node.parent.id.type === AST_NODE_TYPES.Identifier) {
    return O.some(node.parent.id);
  }
  if (node.parent.type === AST_NODE_TYPES.AssignmentExpression && node.parent.right === node && node.parent.operator === "=" && node.parent.left.type === AST_NODE_TYPES.Identifier) {
    return O.some(node.parent.left);
  }
  if (node.parent.type === AST_NODE_TYPES.Property && node.parent.value === node && !node.parent.computed && node.parent.key.type === AST_NODE_TYPES.Identifier) {
    return O.some(node.parent.key);
  }
  if (isOneOf([AST_NODE_TYPES.MethodDefinition, AST_NODE_TYPES.PropertyDefinition])(node.parent) && node.parent.value === node && node.parent.key.type === AST_NODE_TYPES.Identifier) {
    return O.some(node.parent.key);
  }
  if (node.parent.type === AST_NODE_TYPES.AssignmentPattern && node.parent.right === node && node.parent.left.type === AST_NODE_TYPES.Identifier) {
    return O.some(node.parent.left);
  }
  return O.none();
}
...
function getNestedCallExpressions(node) {
  ...
  if (node.type === AST_NODE_TYPES$1.CallExpression) {
