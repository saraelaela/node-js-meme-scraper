...
function getFunctionIdentifier(node) {
  if (node.id) return tools.O.some(node.id);
  if (node.parent.type === types.AST_NODE_TYPES.VariableDeclarator && node.parent.init === node && node.parent.id.type === types.AST_NODE_TYPES.Identifier) {
    return tools.O.some(node.parent.id);
  }
  if (node.parent.type === types.AST_NODE_TYPES.AssignmentExpression && node.parent.right === node && node.parent.operator === "=" && node.parent.left.type === types.AST_NODE_TYPES.Identifier) {
    return tools.O.some(node.parent.left);
  }
  if (node.parent.type === types.AST_NODE_TYPES.Property && node.parent.value === node && !node.parent.computed && node.parent.key.type === types.AST_NODE_TYPES.Identifier) {
    return tools.O.some(node.parent.key);
  }
  if (isOneOf([types.AST_NODE_TYPES.MethodDefinition, types.AST_NODE_TYPES.PropertyDefinition])(node.parent) && node.parent.value === node && node.parent.key.type === types.AST_NODE_TYPES.Identifier) {
    return tools.O.some(node.parent.key);
  }
  if (node.parent.type === types.AST_NODE_TYPES.AssignmentPattern && node.parent.right === node && node.parent.left.type === types.AST_NODE_TYPES.Identifier) {
    return tools.O.some(node.parent.left);
  }
  return tools.O.none();
}
...
function getNestedCallExpressions(node) {
  const callExpressions = [];
  if (node.type === typescriptEstree.AST_NODE_TYPES.CallExpression) {
  ...
  if ("consequent" in node) {
    const chunk = Array.isArray(node.consequent) ? node.consequent.map(getNestedCallExpressions).flat(1) : getNestedCallExpressions(node.consequent);
    callExpressions.push(...chunk);
  }
  if ("alternate" in node && node.alternate !== null) {
    const chunk = Array.isArray(node.alternate) ? node.alternate.map(getNestedCallExpressions).flat(1) : getNestedCallExpressions(node.alternate);
    callExpressions.push(...chunk);
  }
  if ("elements" in node) {
