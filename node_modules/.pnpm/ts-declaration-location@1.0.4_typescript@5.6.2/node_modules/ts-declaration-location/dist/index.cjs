...
function typeMatchesSpecifier(program, specifier, type) {
    ...
    switch (specifier.from) {
        ...
        case "package": {
            return (isTypeDeclaredInPackage(program, declarations, declarationFiles, specifier.package) && !isTypeDeclaredInDefaultLib(program, declarationFiles));
        }
        case "file": {
...
function isTypeDeclaredInLocalFile(program, declarationFiles, globPattern) {
    ...
    return filteredDeclarationFiles.some((declaration) => {
        ...
        return (canonicalPath.startsWith(cwd) && minimatch.minimatch(canonicalPath, globPattern));
    });
}
/**
...
function getCanonicalFilePath(filePath) {
    ...
    const normalizedWithoutTrailingSlash = normalized.endsWith(path__namespace.sep)
